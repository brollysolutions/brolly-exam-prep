...
const unexpectedNavigation = () => { throw new Error('Unexpected navigation during guest rendering'); };
const router = {
  back: unexpectedNavigation, forward: unexpectedNavigation, refresh: unexpectedNavigation,
  hmrRefresh: unexpectedNavigation, push: unexpectedNavigation, replace: unexpectedNavigation,
...
function renderGuest(content: ReactNode) {
  return renderToStaticMarkup(createElement(AppRouterContext.Provider, { value: router },
    createElement(PathnameContext.Provider, { value: '/tests' },
...
test('website navigation has no login link for an anonymous visitor', () => {
  ...
  assert.match(html, /Constable Mock Test/);
...
test('retired login URLs preserve the requested test while preventing redirect loops and external redirects', () => {
  ...
  assert.equal(loginReturnHref('/test/constablemocktest01/solution'), '/test/constablemocktest01/solution');
