...
- `pnpm docker:up` (= `docker compose up -d --build`) builds and starts five services: `web` — the Expo static web export behind nginx — on http://localhost:3201, `api` (FastAPI; uvicorn `--reload` over the bind-mounted `services/api/app`) on http://localhost:8200 with Swagger at http://localhost:8200/docs, `worker` (arq crons), `postgres` (5442) and `redis` (6379). The dev OTP is `123456`.
- Login in Docker: any 10-digit number signs you straight in — the OTP screen was removed on 2026-09-07, so there is no code to enter and no dev code. `POST /v1/auth/phone` creates the account the first time it sees a number.
...
- Change ports without touching the compose file: `WEB_HOST_PORT`, `API_HOST_PORT`, `POSTGRES_HOST_PORT`, `REDIS_HOST_PORT` in a root `.env` or on the command line (`WEB_HOST_PORT=3211 pnpm docker:up`). The api's `CORS_ORIGINS` default follows `PUBLIC_URL` and `WEB_HOST_PORT`; the web image's `EXPO_PUBLIC_API_URL` build arg is `PUBLIC_URL/api` (default `http://localhost:3201/api`) and is baked into the bundle, so a new web port or public URL means rebuilding `web`. Postgres publishes on 5442 (since 2026-09-05) so a native PostgreSQL or another stack on 5432 does not collide (`psql -h 127.0.0.1 -p 5442 -U tslprb`). On this laptop another project holds 3201 (`voice-agent-elevenlabs-console-1`): stop it, or run `WEB_HOST_PORT=3211 pnpm docker:up`. Postgres and Redis are published on 127.0.0.1 only; api and web on all interfaces so a phone on the LAN can reach them (dev OTP is on — do not expose this stack beyond a trusted network).
- The web image is built from the repo root (`apps/mobile/Dockerfile`: node:22-alpine + pnpm 11.25.0 → `expo export --platform web` with `EXPO_PUBLIC_API=http` → `nginx:alpine`, `apps/mobile/nginx.conf`). The browser calls the api through nginx on the web origin, `PUBLIC_URL/api` (`location /api/` proxies to the api service, prefix stripped, `Cache-Control: no-store`), never the compose service name; `http://localhost:8200` stays open for Swagger and tooling. Edits under `apps/mobile` or `packages/*` are not live in the container: `docker compose build web && docker compose up -d web` (or `pnpm docker:up` again). API edits reload through the bind mount.
...
- What goes over the wire today: phone sign-in (`POST /v1/auth/phone`) and the attempt lifecycle (`POST /v1/attempts`, `PATCH …/answers`, `POST …/submit`, `GET /v1/results/{id}`). The test catalogue, the paper and the result analysis are served by `HttpApi` from the in-app fixture bank (`MockApi` fallback) until `GET /v1/tests/{id}/paper` exists. The API's fixture ids (`test-pwt-07`, `q-arith-*`) do not match the app's (`mock-07`, `q-ar-001#n`), so `POST /v1/attempts` answers 404 and the attempt runs on the local clock, as designed — aligning the two banks is an F-17 follow-up, not a Docker one. Because `history.record()` only runs after a successful submit, Home's "papers practised" and "best score" tiles stay empty in Docker until the two fixture banks are aligned.

### Public site: https://mocktest.brollyexamprep.com
- On the server: `git pull`, then `docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build` (= `pnpm docker:prod` where pnpm exists) and once `docker compose exec api alembic upgrade head`. The overlay bakes `https://mocktest.brollyexamprep.com/api` into the web bundle, sets the api's `CORS_ORIGINS` to that origin and gives every service `restart: unless-stopped`.
- A TLS reverse proxy on the server (Caddy, nginx, Traefik) terminates HTTPS for `mocktest.brollyexamprep.com` and forwards everything to `http://127.0.0.1:3201`, passing `Host`. One route is enough: the browser reaches the api on the same origin under `/api`, so no CORS and no second hostname. Caddy: `mocktest.brollyexamprep.com { reverse_proxy 127.0.0.1:3201 }`. Firewall 8200, 5442 and 6379 from the outside.
- Another hostname without touching the overlay: `PUBLIC_URL=https://other.example docker compose up -d --build web api`. Changing the URL always means rebuilding `web`.
- Sign-in is the phone number alone: anyone who types a number is signed in as it, and a stranger who guesses a number gets that account. Keep the site to a trusted audience until a second factor exists (F-17 phase 2).
...
"**Local services (TSLPRB)**: Docker Compose (web on 3201, FastAPI on 8200, postgres:17 on 5442, redis:7 on 6379, arq worker) — dev only, never prod",
```
