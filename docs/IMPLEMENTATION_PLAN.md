...
- [x] Motion helpers (sheet/fade/toast/critical-rail pulse) with reduced-motion
...
- [x] F-28 Brolly rebrand, Phase A (foundation) — semantic tokens on cream with `legacyColors` aliases and contrast/legacy tests; Inter + Playfair Display; every `src/ui` primitive on semantic names (ink primary, gold marks, red outlines, `Rail` for the critical clock, brand palette semantics); the `Brand` lockup on Home/Login/gallery and the full logo on Welcome; light splash, navy/gold app icon, `make-brand-assets.mjs`; docs rewritten. Six commits on main (A1 tokens → A2 fonts → A3a/A3b primitives → A4 brand → A5 splash/icons/docs), each green; fix wave 1 after review (`outline` token, contrast compositor, type ramp, Telugu serif, chip/segment edges, disabled primary, one red toast, ink-tinted rasters, one gold edge on Home, green eligible verdict, `it.failing` guards for Phase C). Phases B (shell screens, F-29), C (content, F-30), D (attempt/result, F-31), E (legacy rename) follow
- [x] F-29 Brolly rebrand, Phase B (screen patterns + shell screens) — `Pill`, `PageHeader`, `BackHeader`, `StatTile`, `MarkerRow`, `ActionBar` and the `EmptyState`/`Skeleton`/`LoadError` trio land in `src/ui` (each with tests and a gallery section), and Welcome, Login, OTP, Post, Category, Home and Profile are rebuilt on them. Five commits on main, each green, plus fix wave 1 (35 items from the code and design reviews: the `ActionBar` inset, `MarkerRow` line-1 alignment + `titleLines`/`trailingLabel`, `Pill dotTone`, the `accentStrong` selected edge, radio semantics on single-choice cards, Welcome's `ScrollView` pager, Home's tagless dotless section heads, the `PageHeader` top offset, and a word-bounded class assertion sweep); no route, store, string or testID changes and 0 new locale keys. Phase C (content screens, F-30) is next

- [x] F-30 Brolly rebrand, Phase C (content screens) — Study, Topic, Library, Paper, Updates, Affairs and Eligibility rebuilt on the P1-P6 patterns, with the three Phase C follow-ups landed first (`Card radius`, a node `MarkerRow.meta`, `titleTestID`/`metaTestID`); a selectable `StatTile` was declined for want of a caller. Five commits on main, each green, plus one fix commit for the three defects the 390 px captures found (the Tests shelf floating in a viewport-tall card, the two Topic callouts on different insets, and a notice's pill shifting 3 px when its gold edge appeared). **Within Phase C's own screens** the `⛌` lock glyph, the `sand` accent and the last four product `Chip tone="label"` sites are gone — all three survive outside it and are Phase D's job: `LOCK_GLYPH` at `AttemptView.tsx:38`, a `sand` kicker at `SolutionsView.tsx:144`, and the tone's own demo in the dev gallery. No route, store, string or testID change beyond the two the spec names, and 0 new locale keys. Fix wave 1 (22 items from the code and design reviews, three commits): `Card startEdge` owns the bar-plus-padding sum the screens were getting wrong by a pixel, the correct answer's disc goes `accentStrong` after measuring 2.16:1 as `accent` (worse than the tick it replaced), the previous-papers shelf drops both ink fills, Study's and Topic's block heads become `Pill`s, Eligibility keeps one pill and suppresses Chromium's focus ring, `SegmentedChips` moves its 48 px floor onto the cell, `MarkerRow`'s a11y contract becomes a prop union, and the Tests shelf gains an empty state (`library.empty`, one new key pair). 873 mobile tests. Phase D (attempt, palette, result, solutions — F-31) is next
...
- 2026-09-05 — Phase B (F-29): screens compose the six `src/ui` patterns instead of laying out their own headers, rows, tiles and footers. A `Pill` labels, a `Chip` is pressed; gold is a fill, an edge or a dot and never a word; the sticky `ActionBar` carries the screen's one ink primary outside the scrolling body
- 2026-09-05 — Phase B fix wave 1 (F-29): a dot on a pill means status, so a section heading carries none; a chrome element owns the inset it sits in (`ActionBar`, like the tab bar) and the offset it starts on (`PageHeader`, 20 px); `hitSlop` is not implemented in react-native-web, so every target grows with padding, never slop; a single-choice card is a `radio` and a static one announces no selection; the category tile stays a `Card` until `StatTile` gets a selectable variant (Phase C), and `Card` gets a `radius` prop in Phase C so a post card can take the `lg` corner the spec asked for
- 2026-09-06 — Phase C (F-30): the **affairs category is a quiet pill** with no gold (the tone Phase B parked); a gold **start edge marks the open card**, not every card in a list; a **card of rows hugs its rows** and the scroller around it takes the height, so a short shelf never floats in a viewport-tall box; the **correct answer's tick sits in a 20 px gold disc**, because a bare gold ✓ on cream is 2.3:1; a **gold index digit stays on the canvas** (`accentInk` is 4.25:1 on a pill's `surface2`); a **node `meta` on `MarkerRow` is silent** in the composed accessibility name, so the row names itself; a **selectable `StatTile` is not built** while no screen selects a tile
