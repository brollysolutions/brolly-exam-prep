...
- [x] F-21 study material — a Study tab between Home and Tests; `packages/fixtures/src/study.ts` carries 11 syllabus topics (arithmetic, reasoning, general studies, Telangana) written in en/te/ur as six block kinds; the reader renders them with a language switcher in its header, hi-vis and sand 3 px start edges that mirror in Urdu, a mark-as-read store (`tslprb.study`) and an ungated hand-off to the sectional drills; free for guests (branch feat/F-21-study)
