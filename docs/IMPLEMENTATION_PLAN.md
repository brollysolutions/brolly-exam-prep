...
- [x] F-29 Brolly rebrand, Phase B (screen patterns + shell screens) — `Pill`, `PageHeader`, `BackHeader`, `StatTile`, `MarkerRow`, `ActionBar` and the `EmptyState`/`Skeleton`/`LoadError` trio land in `src/ui` (each with tests and a gallery section), and Welcome, Login, OTP, Post, Category, Home and Profile are rebuilt on them. Four commits on main, each green; no route, store, string or testID changes and 0 new locale keys. Phase C (content screens, F-30) is next
