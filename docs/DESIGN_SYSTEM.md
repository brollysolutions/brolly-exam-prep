...
saying "selected" is a lie. (`StatTile` has no selectable variant yet; Phase C is to give it
one, so the category grid can stop being a `Card` that looks like a tile.)
