...
- 2026-09-06 — Phase C fix wave 1 (F-30 review). Four rulings, each measured against the build:
  - **One ink fill per screen counts every INSTANCE, including per-row buttons; a repeated per-row action is never the ink fill.** Two ink Practise buttons were 14 % of the Tests shelf card at 15.1:1 while the active filter chip — the only thing saying which shelf you are on — is 1.38:1. Rank inside a repeated pair is the box, not the fill: outlined against bare.
