...
- Fonts: `Inter` (en, with `PlayfairDisplay` for the display roles `title`/`titleLg`/`display`/`wordmark`/`wordmarkSub`), `NotoSansTelugu` (te, with `NotoSerifTelugu` 700 for the same display roles on a 1.5 line-height — Playfair has no Telugu). Numbers (`<Num>`) are always Inter. Use `useTypography()`; never set `fontFamily` directly.
