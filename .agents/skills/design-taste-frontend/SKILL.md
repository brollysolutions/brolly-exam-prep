...
Scope: marketing and landing pages in apps/web; exclude exam screens, authentication, checkout flows, and dashboards. Read the repository AGENTS.md. Local baseline: DESIGN_VARIANCE 3, MOTION_INTENSITY 2, VISUAL_DENSITY 4. Preserve the existing Inter/Playfair fonts, cream/ink/gold tokens, and meaningful question labels. Existing branding overrides upstream font bans and novelty defaults. Dark mode, new animation dependencies, and extra page sections are optional only when the current task calls for them. Treat the upstream checklist as applicable checks, not authorization to expand scope. This is upstream v2 experimental, pinned for reviewable updates.
