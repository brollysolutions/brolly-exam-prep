...
Read the repository AGENTS.md. For this app, review English/Telugu comprehension, text scaling, offline and saved-answer feedback, exam timer visibility, question navigation, and submission recovery. Preserve domain behavior when fixing UI. A review request alone does not authorize changes. Never treat generic item-count rules as a hard limit on exam questions; assess actual recognition and task load. Cite observed behavior and distinguish untested assumptions. Referenced sibling skills are optional and may not be installed.
