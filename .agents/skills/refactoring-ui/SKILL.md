...
Read the repository AGENTS.md and packages/design-tokens before proposing visual changes. Use existing brand tokens rather than generating a replacement palette. Apply hierarchy principles to mobile using existing React Native components; the web/CSS examples are not native implementation instructions. A review request alone does not authorize changes. Scores are heuristic judgments, not evidence of user testing. Referenced sibling skills are optional and may not be installed.
