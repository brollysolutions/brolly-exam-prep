---
name: shadcn
description: Add, inspect, style, and compose shadcn/ui components in the Next.js website at apps/web. Use for shadcn and web component registry tasks; not React Native screens.
---

## Local application configuration

Scope: apps/web only. Read the repository AGENTS.md. Use the installed CLI from apps/web with pnpm exec shadcn. Prefer the official shadcn registry for this setup. Preserve the Brolly theme in lib/theme.ts and existing component customizations. Do not use DOM components in apps/mobile. Codex does not execute shell interpolation in Markdown: run info --json explicitly; project context is not pre-injected. Examples using npx shadcn@latest below should use the installed CLI to respect the lockfile.
...
Run `pnpm exec shadcn info --json` from `apps/web` and inspect the returned context.
